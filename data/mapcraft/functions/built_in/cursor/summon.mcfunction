...
execute store result entity @e[tag=Cursor,tag=Allowed_Movement,distance=..2,limit=1] Attributes[0].Base double 1 run scoreboard players get @s MC_PlayerID
execute as @e[tag=Cursor,tag=Allowed_Movement,distance=..2,limit=1] if data entity @s Attributes[0].Base store result score @s MC_CursorTemp run data get entity @s Attributes[0].Base

function mapcraft:built_in/cursor/width_draw

# Specific rule for team
execute if entity @s[tag=Select] run team join MC_Blue @e[tag=Cursor,tag=One,limit=1,distance=..2,sort=nearest]
execute if entity @s[tag=Draw] run team join MC_Purple @e[tag=Cursor,tag=One,limit=1,distance=..2,sort=nearest]
execute if entity @s[tag=Trigger] run team join MC_Green @e[tag=Cursor,tag=One,limit=1,distance=..2,sort=nearest]
execute if entity @s[tag=Cutscene] run team join MC_Yellow @e[tag=Cursor,tag=One,limit=1,distance=..2,sort=nearest]

execute if entity @s[tag=Select] run team join MC_Yellow @e[tag=Cursor,tag=Two,limit=1,distance=..2,sort=nearest]
execute if entity @s[tag=Copy] run team join MC_DarkBlue @e[tag=Cursor,tag=Copy,limit=1,distance=..2,sort=nearest]
